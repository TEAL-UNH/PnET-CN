...
void pnet_model::ReadInput(site_struct* site,veg_struct* veg,share_struct* share, char* inputname)
{
	...
	if (site->distyrs>0) 
	{
		for (i=1;i<=site->distyrs;i++)
		{
			...
			fscanf(f_input_load, "%d %lf %lf %lf %lf \n",
					&site->distyear[i],
...
void pnet_model::WriteoutMo(site_struct* site,veg_struct* veg,clim_struct* clim, share_struct* share,int rstep,FILE* &fileoutM)
{
	...
	if (rstep==1)
	{
		...
		sprintf(outputmo,"%s%s%s",exePath,PathOutSite,"Output_monthly.csv");

		fileoutM = fopen(outputmo, "w" );
...
void pnet_model::WriteoutYr(clim_struct* clim,out_struct* out)
{
	...
	{	
		...
		fprintf(fileoutY,"%5d, ",clim->year[12*i]);  // sequence of year	double*	folm;
		...
		fprintf(fileoutY,"%7.1f, %6.1f, %6.1f, %6.1f, %6.1f, %6.1f,",out->prec[i],out->evap[i],
		...
		fprintf(fileoutY,"%7.2f, %7.2f,",out->v1[i],out->v2[i]);   // spare variables for future use
