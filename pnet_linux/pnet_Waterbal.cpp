...
void pnet_model::Waterbal(site_struct* site, veg_struct* veg, clim_struct* clim, int rstep, share_struct* share)
{
	...
	double	Trans, TotSoilMoistEff, Transd, CanopyDO3, CanopyGrossPsnAct;
	...
	if ((clim->O3[rstep]>0) && (share->CanopyGrossPsn>0))
	{
		if (share->WUEO3Eff == 0)
		{
			...
			CanopyDO3 = share->CanopyDO3Pot + ((1 - share->CanopyDO3Pot) * (1 - share->DWater));  // drought offset to O3 effect
		}
		else
		{
		...
		share->DroughtO3Frac = share->CanopyDO3Pot / CanopyDO3;  // relative drought offset
	}
	else
	{
		CanopyDO3 = 1;
	...
	share->NetPsnMo = (CanopyGrossPsnAct - (share->DayResp + share->NightResp) * share->FolMass) * share->dayspan * CanopyDO3;
